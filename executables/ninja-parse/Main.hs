...
-- File: executables/ninja-parse/Main.hs
--
-- License:
...
parseOptions = Opt.unwrapRecord (Text.pack ("ninja-parse version " <> version))
...
prettyNinja _inputPath (input, output) _color = do
  ...
  case parsed of
    ...
    Right x -> Text.hPutStr output (Pretty.prettyNinja (void x))
