...
with rec {
    ...
    mkJobset ((removeAttrs args ["url" "branch"]) // {
      ...
      inputs = {
        src = mkFetchGithub (
          if url != null
          then "${url} ${branch}"
          else "https://github.com/awakesecurity/language-ninja ${branch}");
      };
    }));
  ...
  primaryJobsets = {
    "master" = mkProjectJobset {
      branch = "master";
      description = "awakesecurity/language-ninja master";
    };
    
    "staging" = mkProjectJobset {
      branch = "staging";
      description = "awakesecurity/language-ninja staging";
    };
  };
